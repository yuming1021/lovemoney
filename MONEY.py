...
# ==========================================
# 🚀 核心優化：雲端快取掃描引擎 (手機不卡頓的秘密)
# 讓伺服器每 60 秒才算一次，手機直接秒拿結果
# ==========================================
...
def scan_whole_market(min_volume, min_stars):
    ...
    if recommended_list:
        ...
        return df_picks.sort_values(by="score_raw", ascending=False).drop(columns=['score_raw']), get_tw_time()
    else:
...
if st.session_state.app_mode == "🤖 全市場自動監控推薦":
    ...
    with st.sidebar:
        ...
        # 雲端版建議拉長更新間隔保護手機
        refresh_rate = st.slider("重新掃描間隔 (秒)", min_value=30, max_value=120, value=60)

    status_placeholder = st.empty()
    table_placeholder = st.empty()

    while True:
        status_placeholder.info("⚡ 雲端引擎正在同步最新資料...")
        ...
        status_placeholder.success(f"✅ 數據同步完成！為避免手機卡頓，將於 {refresh_rate} 秒後更新。")
...
# ==========================================
    ...
    with st.sidebar:
        ...
        if auto_refresh_search:
            search_refresh_rate = st.slider("個股刷新間隔 (秒)", min_value=15, max_value=60, value=30)
            ...
            with chart_placeholder.container():
                try:
                    ...
                    if not df_chart.empty and len(df_chart) >= 50:
                        ...
                        # 降低圖表高度以適應手機螢幕
                        fig.update_layout(xaxis_rangeslider_visible=False, height=500, margin=dict(l=10, r=10, t=10, b=10), hovermode='x unified')
                        ...
                        if star_count == 5:
                            ai_pred = "🔥 **型態走勢：極度看漲 (Strong Buy)**\n\n技術面完美，均線強勢多頭排列且伴隨量增，預測短線將發動大波段行情。"
                        elif star_count == 4:
                            ai_pred = "🚀 **型態走勢：穩健看漲 (Buy)**\n\n趨勢明顯轉強，長短天期均線給予支撐。預期將沿 5 日線向上推升。"
                        elif star_count == 3:
                            ai_pred = "📈 **型態走勢：溫和偏多 (Accumulate)**\n\n已站上關鍵支撐，動能尚未完全爆發。預期短期將震盪走高，可逢低佈局。"
                        elif star_count == 2:
                            ai_pred = "➖ **型態走勢：中性盤整 (Hold)**\n\n多空勢均力敵，正處於區間震盪階段。建議靜待帶量突破方向再行動。"
                        elif star_count == 1:
                            ai_pred = "📉 **型態走勢：轉弱疑慮 (Underperform)**\n\n跌破重要支撐，均線下彎。短線面臨回檔壓力，建議退場觀望。"
                        else:
                            ai_pred = "🚨 **型態走勢：極度弱勢 (Strong Sell)**\n\n空頭格局成型，賣壓沉重。極有可能持續探底，請嚴格避開！"
